...
class TPPRunner():
    ...
    def _gen_model(self, data_loader, **kwargs):
        ...
        test_result = self.run_one_epoch_probability(data_loader, RunnerPhase.PREDICT)
    ...
    def run_one_epoch(self, data_loader, phase):
        ...
        if phase in [RunnerPhase.TRAIN, RunnerPhase.VALIDATE]:
            ...
            for batch in data_loader:
                ...
                print(np.array(batch_pred).shape)
                epoch_pred.append(batch_pred)
                epoch_label.append(batch_label)
    ...
    def run_one_epoch_intensity(self, data_loader, phase):
        ...
        for batch in data_loader:
            batch_intensities, batch_exp_numbers, batch_label = self.model_wrapper.run_batch_intensity(batch, phase=phase)
            intensities_pred.append(batch_intensities)
            exp_numbers_pred.append(batch_exp_numbers)
            epoch_label.append(batch_label)

        if phase == RunnerPhase.PREDICT:
            metrics_dict.update({'intensities_pred': intensities_pred, 'exp_numbers_pred': exp_numbers_pred, 'label': epoch_label})

        return metrics_dict
    
    def run_one_epoch_probability(self, data_loader, phase):
        """Run one complete epoch and store the intensity values.
        ...
        for batch in data_loader:
            batch_probs, batch_label = self.model_wrapper.run_batch_probability(batch, phase=phase)
